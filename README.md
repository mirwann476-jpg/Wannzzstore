    ...
    <script>
        ...
        function setGeminiLoading(isLoading) {
            if (isLoading) {
                ...
                // Atur ulang teks tombol ke nama item yang baru
                if (selectedDiamond) {
        ...
        /**
         * Memanggil Gemini API dengan retry dan exponential backoff.
         */
        async function getGeminiIdeas() {
            ...
            const apiKey = ""; // API key akan di-inject oleh environment
            const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`;
            
            // Prompt disesuaikan untuk Mobile Legends
        ...
        function init() {
            ...
            buyButtonEl.addEventListener('click', () => {
                ...
                console.log('Pesanan Dibuat:', {
                    userId,
                    zoneId,
                    item: selectedDiamond.name,
                    payment: selectedPayment.name,
                    price: selectedDiamond.price,
                    email
                });
                
                showMessage(`Pesanan ${selectedDiamond.name} berhasil! Cek email ${email} untuk instruksi pembayaran.`, 'success');
                
                // Di sini Anda biasanya akan mengarahkan ke halaman pembayaran atau mengirim data ke server
            });
        }
